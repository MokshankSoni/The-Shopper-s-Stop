...
const NavBar = () => { 
  ...
  return (
    <div className="flex items-center justify-between py-5 font-medium"> 
     <Link to ='/'><img src = "/src/assets/logo.png" className="w-36" alt="Logo" /> </Link> 
      ...
      <div className='flex items-center gap-6'>
          ...
          <img  onClick={() => setVisible(true)} src="/src/assets/menu_icon.png" className='w-5 cursor-pointer sm:hidden' alt="" />
      </div> 
      {/* sidebar menu for smaller screen */}
      <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${visible ? 'w-full' : 'w-0'}`}>
        <div className='flex flex-col text-gray-600'>
          <div onClick={()=> setVisible(false)} className='flex items-center gap-4 p-3 cursor-pointer'>
            <img className='h-4 rotate-180' src="/src/assests/dropdown_icon.png" alt="" />
            <p>Back</p>
          </div>
          ...
          <NavLink onClick={() => setVisible(false)} className='py-2 p1-6 border' to='/AboutUs'>AboutUs</NavLink>
          <NavLink onClick={() => setVisible(false)} className='py-2 p1-6 border' to='/HomeUs'>ContactUs</NavLink>
        </div>
      </div>
    </div>
  ); 
